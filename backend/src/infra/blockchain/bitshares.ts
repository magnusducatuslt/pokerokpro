...
export class Bitshares {
    ...
    public findUser(username:string): Promise<typeof Result>{
        //@ts-ignore
        return axios.post<typeof fullAccounwDTO>(URL,{"method": "call","params":[0,"get_full_accounts",[[username],true]],"id": 9})
            .then(result => {
                return result.data.result[0][1]
            })
    }

    public validateUser(data:{username:string,password:string,pubKey:string}):Boolean{
        const roles = ["active","owner","memo"];
        const parsedKey = data.pubKey.slice(4);

        return roles.some(role=>this.generateKeyFromPassword(data.username,role,data.password).slice(4) === parsedKey)
    ...
    public async receiveRating(data:Pick<AccountAttributes,"accountName">): Promise<RatingFromTotalPoker>{
        return axios.get<RatingFromTotalPoker>(`http://api.prod.totalpoker.io:8403/leaderboard/api/v1.0/current?player_name=${data.accountName}&request_id=0`)
